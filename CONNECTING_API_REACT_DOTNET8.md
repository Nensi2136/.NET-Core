...
### Frontend packages (`card printing/package.json`)

#### Dependencies

- **react**: Core UI library for building component-based interfaces.
- **react-dom**: Renders React components to the browser DOM.
- **react-router-dom**: Client-side routing for navigating pages without full reloads.
- **axios**: Makes HTTP API requests to the backend with a simple promise-based API.
- **lucide-react**: Provides ready-to-use SVG icon components for the UI.
- **html2canvas**: Captures HTML content and converts it into an image (canvas) for export/print flows.
- **jspdf**: Generates PDF files in the browser (for downloads/printing).
- **cors**: Helps handle cross-origin access when needed during development tooling.

#### DevDependencies

- **vite**: Fast dev server and bundler for modern frontend builds.
- **@vitejs/plugin-react**: Adds React/JSX support and optimized tooling to Vite.
- **typescript**: Adds static typing for safer, more maintainable code.
- **@types/react**: TypeScript type definitions for React.
- **@types/react-dom**: TypeScript type definitions for React DOM.
- **eslint**: Lints code to catch bugs and enforce consistent style.
- **@eslint/js**: Base ESLint rules for JavaScript projects.
- **typescript-eslint**: ESLint tooling to lint TypeScript code correctly.
- **eslint-plugin-react-hooks**: Enforces correct usage of React Hooks rules.
- **eslint-plugin-react-refresh**: Improves DX by validating React Fast Refresh boundaries.
- **tailwindcss**: Utility-first CSS framework for quickly styling UI.
- **postcss**: CSS processing pipeline used by Tailwind and modern CSS transforms.
- **autoprefixer**: Automatically adds vendor prefixes for cross-browser CSS support.
- **globals**: Provides known global variable definitions for linting.

### Backend packages (`cardprintingAPI/cardprintingAPI.csproj`)

- **Microsoft.EntityFrameworkCore**: ORM for querying and saving data using C# objects.
- **Microsoft.EntityFrameworkCore.SqlServer**: SQL Server database provider for EF Core.
- **Microsoft.EntityFrameworkCore.Design**: Design-time support for scaffolding and migrations in development.
- **Microsoft.EntityFrameworkCore.Tools**: CLI tooling support for EF Core commands (migrations, updates).
- **Newtonsoft.Json**: JSON serialization/deserialization for flexible JSON handling.
- **Swashbuckle.AspNetCore**: Generates Swagger/OpenAPI docs and provides Swagger UI for testing APIs.
